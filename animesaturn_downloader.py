...
def render_player_page(player_page_url: str) -> str:
    ...
    try:
        ...
        if retries > DOWNLOAD_MAX_RETRIES:
            ...
            logging.warning(
                f"Error while rendering page {player_page_url}, retrying...")
...
def download_resource(resource_url: str):
    ...
    try:
        ...
        if retries > DOWNLOAD_MAX_RETRIES:
            ...
            logging.warning(
                f"Error while downloading {resource_url}, retrying...")
            time.sleep(SLEEP_SECONDS_ON_ERROR)
...
def main(main_url: str, ep_range_start: int, ep_range_end: int):
    ...
    for link in links:
        ...
        if mp4_url:
            ...
            stream_playlist_url = stream_playlist_url.replace(
                "playlist.m3u8", f"{STREAM_RESOLUTION}/playlist_{STREAM_RESOLUTION}.m3u8")
            playlist = requests.get(stream_playlist_url).text
            playlist_urls = list()
            for line in playlist.split("\n"):
                if line.endswith(".ts"):
                    playlist_urls.append(stream_playlist_url.replace(
                        f"playlist_{STREAM_RESOLUTION}.m3u8", line))
